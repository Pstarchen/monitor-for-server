...
if [[ "${1:-}" == "pull" && "${TEST_FAIL_AGENT_PULLS:-0}" == "1" ]]; then
  ...
  if [[ "${4:-}" == *'org.opencontainers.image.version'* ]]; then
    printf '%s\n' "${TEST_AGENT_IMAGE_VERSION:-v1.20.22}"
  else
...
server_url=https://monitor.example.com
run_installer 1
grep -F 'docker pull ghcr.io/pstarchen/monitor-for-server-agent:v1.20.22' "${log_file}" >/dev/null
grep -F 'timeout 45s docker pull ghcr.io/pstarchen/monitor-for-server-agent:v1.20.22' "${log_file}" >/dev/null
...
run_installer 1
grep -F 'docker build --pull --build-arg VERSION=v1.20.22 --tag ghcr.io/pstarchen/monitor-for-server-agent:v1.20.22 https://gitee.com/starchen520/monitor-for-server.git#v1.20.22:agent' "${log_file}" >/dev/null
grep -F 'docker build --pull --build-arg VERSION=v1.20.22 --tag ghcr.io/pstarchen/monitor-for-server-agent:v1.20.22 https://github.com/Pstarchen/monitor-for-server.git#v1.20.22:agent' "${log_file}" >/dev/null
