...
function New-TestFixture([string] $Name) {
    ...
    foreach ($platform in @(
        ...
        $assetName = "xingchen-agent_1.20.18_$($platform.OS)_$($platform.Arch).$($platform.Extension)"
...
function Invoke-Update($Fixture, [string[]] $Arguments, [hashtable] $Environment = @{}, [bool] $IncludeBundle = $true) {
    ...
    $names = @(
        ...
        'TEST_PULL_SUCCESS_PREFIX', 'TEST_RUNNING_VERSION', 'TEST_RUNNING_IMAGE_PREFIX', 'XINGCHEN_SOURCE_REPOSITORIES', 'XINGCHEN_UPDATE_MIN_FREE_BYTES'
    )
    $saved = @{}
...
public static class FakeDocker
{
    ...
    public static int Main(string[] args)
    {
        ...
        if (args[0] == "inspect")
        {
            ...
            else if (template.Contains(".Config.Image")) Console.WriteLine(EnvironmentValue("TEST_RUNNING_IMAGE_PREFIX") != "" ? EnvironmentValue("TEST_RUNNING_IMAGE_PREFIX") + service + ":" + EnvironmentValue("TEST_RUNNING_VERSION") : "registry.old.local/xingchen/" + service + ":v1.20.14");
...
function global:docker {
    ...
    if ($dockerArguments[0] -eq 'inspect') {
        ...
        elseif ($template -like '*.Config.Image*') {
            if ($env:TEST_RUNNING_IMAGE_PREFIX) { Write-Output "$env:TEST_RUNNING_IMAGE_PREFIX$service`:$env:TEST_RUNNING_VERSION" }
            else { Write-Output "registry.old.local/xingchen/$service`:v1.20.14" }
        }
...
'@
    ...
    Assert-True ($envAfter.Contains('XINGCHEN_TARGET_VERSION="v1.20.18"') -and $envAfter.Contains('XINGCHEN_NETWORK_MODE="offline"') -and $envAfter.Contains('XINGCHEN_RELEASE_MANIFEST_SHA256="')) "Apply did not persist offline candidate settings.`n$envAfter"
    ...
    $bundleImages = @(
        ...
        'ghcr.io/pstarchen/monitor-for-server-agent:v1.20.18',
        'postgres:16-alpine',
        'redis:7.4-alpine'
    )
...
'@
    ...
    Assert-True ($result.Log.Contains('docker pull ghcr.io/pstarchen/monitor-for-server-server:v1.20.18')) "Online Check did not pull the expected candidate image.`n$($result.Log)"
    ...
    $sameVersionEnvironment = @{
        TEST_RUNNING_VERSION = 'v1.20.18'
        TEST_RUNNING_IMAGE_PREFIX = 'ghcr.io/pstarchen/monitor-for-server-'
        TEST_PULL_SUCCESS_PREFIX = 'ghcr.io/'
    }
    $result = Invoke-Update $fixture @('-Apply', '-NoMirror', '-NoSourceFallback') $sameVersionEnvironment $false
    Assert-True ($result.Status -eq 0) "Same-version no-op failed.`n$($result.Output)"
    Assert-True ($result.Log -notmatch '(?m)^docker pull |(?m)^docker compose .*\bup\b') 'Unchanged same-version deployment was restarted.'
    $sameVersionEnvFile = Join-Path $fixture.Project '.env'
    Write-Utf8 $sameVersionEnvFile ([System.IO.File]::ReadAllText($sameVersionEnvFile).Replace('ghcr.io/pstarchen/monitor-for-server-server:v1.20.18', 'ccr.ccs.tencentyun.com/xc_monitor/monitor-for-server-server:v1.20.18'))
    $sameVersionEnvironment.TEST_PULL_SUCCESS_PREFIX = ''
    $result = Invoke-Update $fixture @('-Apply', '-Offline') $sameVersionEnvironment $false
    Assert-True ($result.Status -eq 0) "Same-version source migration failed.`n$($result.Output)"
    Assert-True ($result.Log -match '(?m)^docker compose .*\bup\b') 'Same-version source migration did not recreate Controller services.'
...
'@
    $managedMissingImages = @(
        'ccr.ccs.tencentyun.com/xc_monitor/monitor-for-server-postgres:v1.20.18',
        'ccr.ccs.tencentyun.com/xc_monitor/monitor-for-server-redis:v1.20.18'
    ) -join ','
    $managedEnvironment = @{
    ...
    foreach ($dependency in @('POSTGRES', 'REDIS')) {
        $targetImage = "ccr.ccs.tencentyun.com/xc_monitor/monitor-for-server-$($dependency.ToLowerInvariant()):v1.20.18"
...
'@
    ...
    Assert-True ($customEnvAfter -notmatch 'registry\.example/base/(?:postgres|redis):v1\.20\.18') 'Custom dependency tag was rewritten to the Controller target version.'
...
'@
    ...
    foreach ($service in @('setup', 'server', 'web')) {
        $internalImage = "registry.internal.example/pstarchen/monitor-for-server-$service`:v1.20.18"
        $logicalImage = "ghcr.io/pstarchen/monitor-for-server-$service`:v1.20.18"
...
XINGCHEN_NETWORK_MODE="internal"
XINGCHEN_TARGET_VERSION="v1.20.18"
XINGCHEN_SETUP_IMAGE="registry.internal.example/xingchen/setup:v1.20.18"
XINGCHEN_SERVER_IMAGE="registry.internal.example/xingchen/server:v1.20.18"
XINGCHEN_WEB_IMAGE="registry.internal.example/xingchen/web:v1.20.18"
XINGCHEN_POSTGRES_IMAGE="registry.internal.example/library/postgres:16-alpine"
XINGCHEN_REDIS_IMAGE="registry.internal.example/library/redis:7.4-alpine"
'@
    $result = Invoke-Update $fixture @('-Check') @{} $false
