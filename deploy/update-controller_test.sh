...
: > "${log_file}"
run_update --check
grep -F 'docker pull ghcr.io/pstarchen/monitor-for-server-server:v1.20.13' "${log_file}" >/dev/null
grep -F 'timeout 180s docker pull ghcr.io/pstarchen/monitor-for-server-server:v1.20.13' "${log_file}" >/dev/null
...
grep -F 'timeout 7s docker pull registry.internal.example/pstarchen/monitor-for-server-server:v1.20.13' "${log_file}" >/dev/null
grep -F 'timeout 11s docker pull ghcr.io/pstarchen/monitor-for-server-server:v1.20.13' "${log_file}" >/dev/null
...
grep -E 'docker tag xingchen-controller-source-[^ ]+-1:candidate ghcr.io/pstarchen/monitor-for-server-server:v1.20.13' "${log_file}" >/dev/null
...
grep -F 'docker pull ghcr.io/pstarchen/monitor-for-server-web:v1.20.13' "${log_file}" >/dev/null
...
grep -F 'docker tag sha256:old-image ghcr.io/pstarchen/monitor-for-server-server:v1.20.13' "${log_file}" >/dev/null
...
printf '%s\n' 'POSTGRES_PASSWORD="test-only"' 'XINGCHEN_TARGET_VERSION="v1.20.13"' > "${offline_root}/.env"
: > "${log_file}"
env "PATH=${fake_bin}:/usr/bin:/bin" "TEST_LOG=${log_file}" "CONTROLLER_AGENT_ENABLED=false" \
  "TEST_FAIL_ALL_PULLS=true" "TEST_IMAGE_VERSION=v1.20.13" bash "${offline_root}/deploy/update-controller.sh" --check --offline
if grep -Eq '^docker (pull|build) ' "${log_file}"; then
  echo 'Offline check attempted a registry pull or remote build.' >&2
  exit 1
fi
grep -F 'docker image inspect ghcr.io/pstarchen/monitor-for-server-server:v1.20.13' "${log_file}" >/dev/null

: > "${log_file}"
if env "PATH=${fake_bin}:/usr/bin:/bin" "TEST_LOG=${log_file}" "CONTROLLER_AGENT_ENABLED=false" \
  "TEST_MISSING_LOCAL_IMAGE=true" "TEST_IMAGE_VERSION=v1.20.13" bash "${offline_root}/deploy/update-controller.sh" --check --offline; then
...
: > "${log_file}"
if env "PATH=${fake_bin}:/usr/bin:/bin" "TEST_LOG=${log_file}" "CONTROLLER_AGENT_ENABLED=false" \
  "TEST_MISSING_LOCAL_IMAGE_MATCH=postgres:16-alpine" "TEST_IMAGE_VERSION=v1.20.13" bash "${offline_root}/deploy/update-controller.sh" --check --offline; then
...
: > "${log_file}"
if env "PATH=${fake_bin}:/usr/bin:/bin" "TEST_LOG=${log_file}" "CONTROLLER_AGENT_ENABLED=false" \
  "TEST_RUNNING_VERSION=v1.20.13" bash "${downgrade_root}/deploy/update-controller.sh" --check --no-mirror; then
...
printf '%s\n' 'POSTGRES_PASSWORD="test-only"' 'XINGCHEN_TARGET_VERSION="v1.20.13"' > "${same_root}/.env"
: > "${log_file}"
env "PATH=${fake_bin}:/usr/bin:/bin" "TEST_LOG=${log_file}" "CONTROLLER_AGENT_ENABLED=false" \
  "TEST_RUNNING_VERSION=v1.20.13" bash "${same_root}/deploy/update-controller.sh" --apply --no-mirror
