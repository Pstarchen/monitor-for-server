...
for build_option in --build --source-build; do
  if "${test_bash}" "${source_installer}" --network-mode internal "${build_option}" >/dev/null 2>&1; then
...
export TEST_REALPATH_COMMAND="$(command -v realpath)"
cat > "${fake_bin}/realpath" <<'SCRIPT'
#!/usr/bin/env bash
set -euo pipefail
# Run the full online/offline suite with the option-free contract required by
# old Setup's BusyBox. The separate runtime suite also runs the real applet.
[[ "$#" -eq 1 && "$1" == /* ]] || {
  echo 'realpath must receive one absolute operand and no GNU options.' >&2
  exit 1
}
exec "${TEST_REALPATH_COMMAND}" "$1"
SCRIPT
chmod +x "${fake_bin}/realpath"

(
  export PATH="${fake_bin}:/usr/bin:/bin"
  source <(awk '/^resolve_existing_path\(\)/ { capture = 1 } capture { print } capture && /^}/ { exit }' "${source_updater}")
  path_root="${temp_dir}/existing paths"
  mkdir -p "${path_root}/-directory"
  printf 'existing path\n' > "${path_root}/-directory/-file"
  [[ "$(resolve_existing_path "${path_root}/-directory/-file")" == "${path_root}/-directory/-file" ]]
  for absent in "${path_root}/missing" "${path_root}/missing/child"; do
    if resolve_existing_path "${absent}" >/dev/null 2>&1; then
      echo 'Portable path resolution accepted a missing path.' >&2
      exit 1
    fi
  done
  if (cd -- "${path_root}" && resolve_existing_path '-directory/-file') >/dev/null 2>&1; then
    echo 'Portable path resolution accepted a relative operand.' >&2
    exit 1
  fi
  ln -s "${path_root}/missing" "${path_root}/dangling" 2>/dev/null || true
  if [[ -L "${path_root}/dangling" ]] && resolve_existing_path "${path_root}/dangling" >/dev/null 2>&1; then
    echo 'Portable path resolution accepted a dangling symlink.' >&2
    exit 1
  fi
)
...
if [[ "${1:-}" == "image" && "${2:-}" == "inspect" ]]; then
  inspected="${@: -1}"
  if [[ "${TEST_MISSING_LOCAL_IMAGE:-false}" == "true" ||
        ( -n "${TEST_MISSING_LOCAL_IMAGE_MATCH:-}" && "${inspected}" == *"${TEST_MISSING_LOCAL_IMAGE_MATCH}"* ) ||
        ( -n "${TEST_MISSING_LOCAL_IMAGE_REGEX:-}" && "${inspected}" =~ ${TEST_MISSING_LOCAL_IMAGE_REGEX} ) ]]; then
    exit 1
  fi
fi
...
run_update() {
  env "PATH=${fake_bin}:/usr/bin:/bin" "TEST_LOG=${log_file}" "CONTROLLER_AGENT_ENABLED=${TEST_CONTROLLER_AGENT_ENABLED:-false}" \
    ...
    "${test_bash}" "${TEST_UPDATER:-${updater}}" "$@"
}
...
: > "${log_file}"
run_update --check
grep -F 'docker pull ghcr.io/pstarchen/monitor-for-server-server:v1.20.22' "${log_file}" >/dev/null
grep -F 'timeout 180s docker pull ghcr.io/pstarchen/monitor-for-server-server:v1.20.22' "${log_file}" >/dev/null
...
grep -F 'timeout 7s docker pull registry.internal.example/pstarchen/monitor-for-server-server:v1.20.22' "${log_file}" >/dev/null
grep -F 'timeout 11s docker pull ghcr.io/pstarchen/monitor-for-server-server:v1.20.22' "${log_file}" >/dev/null
...
grep -E 'docker tag xingchen-controller-source-[^ ]+-1:candidate ghcr.io/pstarchen/monitor-for-server-server:v1.20.22' "${log_file}" >/dev/null
...
grep -F 'docker pull ghcr.io/pstarchen/monitor-for-server-web:v1.20.22' "${log_file}" >/dev/null
...
managed_dependency_root="${temp_dir}/managed-dependency-project"
create_upgrade_project "${managed_dependency_root}"
printf '%s\n' \
  'POSTGRES_PASSWORD="test-only"' \
  'COMPOSE_PROJECT_NAME="xingchen-monitor"' \
  ...
  'XINGCHEN_REDIS_IMAGE="ccr.ccs.tencentyun.com/xc_monitor/monitor-for-server-redis:v1.20.16"' > "${managed_dependency_root}/.env"
: > "${log_file}"
...
set -e
if [[ "${managed_rollback_status}" -ne 10 ]]; then
  echo "Managed dependency rollback returned ${managed_rollback_status}, want 10." >&2
  exit 1
fi
grep -F 'XINGCHEN_POSTGRES_IMAGE="ccr.ccs.tencentyun.com/xc_monitor/monitor-for-server-postgres:v1.20.16"' "${managed_dependency_root}/.env" >/dev/null
...
grep -q '^docker compose .* up -d --force-recreate --wait' "${log_file}"

custom_dependency_root="${temp_dir}/custom-dependency-project"
create_upgrade_project "${custom_dependency_root}"
printf '%s\n' \
  'POSTGRES_PASSWORD="test-only"' \
  'COMPOSE_PROJECT_NAME="xingchen-monitor"' \
  'XINGCHEN_TARGET_VERSION="v1.20.18"' \
...
grep -F 'docker tag sha256:old-image ghcr.io/pstarchen/monitor-for-server-server:v1.20.22' "${log_file}" >/dev/null
...
printf '%s\n' \
  'POSTGRES_PASSWORD="test-only"' \
  'XINGCHEN_TARGET_VERSION="v1.20.14"' \
  'XINGCHEN_POSTGRES_IMAGE="registry.example/base/postgres:16"' \
  'XINGCHEN_REDIS_IMAGE="registry.example/base/redis:7.4"' > "${same_root}/.env"
cp "${same_root}/.env" "${same_root}/env.before"
: > "${log_file}"
env "PATH=${fake_bin}:/usr/bin:/bin" "TEST_LOG=${log_file}" "CONTROLLER_AGENT_ENABLED=false" \
