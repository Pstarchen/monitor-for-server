...
describe('realtime events', () => {
  it('parses a typed device event', () => {
    const event = new CustomEvent('xingchen:realtime', { detail: { type: 'metric.updated', payload: { deviceId: 'server-a' } } })
    expect(realtimeEvent(event)).toEqual({ type: 'metric.updated', payload: { deviceId: 'server-a' } })
  })

  it('filters by event type and device', () => {
    const event = new CustomEvent('xingchen:realtime', { detail: { type: 'alert.opened', payload: { deviceId: 'server-a' } } })
  ...
  it('ignores malformed payloads', () => {
    expect(realtimeEvent(new CustomEvent('xingchen:realtime', { detail: { type: 'metric.updated' } }))).toBeNull()
    expect(realtimeEvent(new CustomEvent('xingchen:realtime', { detail: 'invalid' }))).toBeNull()
  })
})
