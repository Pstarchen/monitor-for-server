...
describe('buildAgentInstallCommand', () => {
  it('builds a short controller-hosted Linux install command', () => {
    ...
    expect(command).toContain("env XINGCHEN_SERVER='https://monitor.example.com' XINGCHEN_DEVICE_ID='device-1' XINGCHEN_AGENT_KEY='agent'\"'\"'key' ./xingchen-agent.sh --interval 30s")
  ...
  it('builds a Windows command for the selected source and clears the temporary key', () => {
    ...
    expect(command).toContain("$env:XINGCHEN_AGENT_KEY = 'agent''key'")
    expect(command).toContain('Remove-Item Env:XINGCHEN_AGENT_KEY')
