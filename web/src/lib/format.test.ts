...
describe('safeLocalPath', () => {
  ...
  it('chooses a readable shared unit for chart throughput', () => {
    ...
    expect(rateScale(1024)).toEqual({ divisor: 1024, unit: 'KiB/s' })
    expect(rateScale(1024 ** 2 * 3)).toEqual({ divisor: 1024 ** 2, unit: 'MiB/s' })
    expect(rateScale(Number.MAX_SAFE_INTEGER)).toEqual({ divisor: 1024 ** 4, unit: 'TiB/s' })
  })
})
