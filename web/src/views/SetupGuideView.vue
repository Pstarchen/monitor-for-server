        ...
        <template v-else>
          ...
          <form class="setup-form" novalidate @submit.prevent="nextStep">
            <div v-if="currentStep === 0" class="setup-form-section">
              ...
              <p class="setup-inline-note"><KeyRound :size="15" />地址不会被安装器改写。MySQL 和总终端在同一台宝塔主机时，不要填写公网 IP 或 localhost；请在宿主机执行 <code>docker network inspect guanlan-monitor_monitor</code>，使用命令返回的 Gateway 和 MySQL 端口。若提示错误码 1130，请给账号增加实际 Docker 网段来源授权。</p>
