...
const canEditWindow = (window: MaintenanceWindow) => auth.user?.role === 'ADMIN'
  || (auth.user?.role === 'OPERATOR' && Boolean(window.scopeDeviceId) && auth.canManageAlerts(window.scopeDeviceId!))
...
function openCreate() {
  ...
  Object.assign(form, { name: '', deviceId: auth.user?.role === 'ADMIN' ? '' : alertDevices.value[0]?.id || '', ruleId: null, range: [start, end], timezone: localTimezone, recurrence: 'NONE', repeatUntil: null, reason: '', enabled: true })
  dialog.value = true
}
...
function openEdit(window: MaintenanceWindow) {
  ...
  Object.assign(form, {
    name: window.name,
    deviceId: window.deviceId ?? (auth.user?.role === 'OPERATOR' ? window.scopeDeviceId ?? '' : ''),
...
<template>
  <section>
    <PageHeader eyebrow="MAINTENANCE" title="维护静默" description="在发布、迁移或检修期间保留告警事件，同时暂停通知；窗口结束后仍未恢复的故障会自动补发通知。">
      <template #actions><el-button @click="load"><RefreshCw :size="16" />刷新</el-button><el-button v-if="canCreate" type="primary" class="button-press" @click="openCreate"><Plus :size="16" />新建窗口</el-button></template>
    </PageHeader>
    ...
    <el-dialog v-model="dialog" :title="editingId ? '编辑维护窗口' : '新建维护窗口'" width="min(620px, calc(100vw - 28px))" destroy-on-close>
      <el-form label-position="top">
        ...
        <div class="form-grid two-fields">
          <el-form-item label="设备范围"><el-select v-model="form.deviceId" :clearable="auth.user?.role === 'ADMIN'" :placeholder="auth.user?.role === 'ADMIN' ? '全部设备' : '选择设备'"><el-option v-if="auth.user?.role === 'ADMIN'" label="全部设备" value="" /><el-option v-for="device in alertDevices" :key="device.id" :label="device.name" :value="device.id" /></el-select></el-form-item>
