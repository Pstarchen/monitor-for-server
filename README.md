...
- Linux/Windows Agent 采集 CPU、内存、磁盘、网络、进程、温度和自定义服务状态；Linux 可选读取 Docker socket 展示容器资源，并在具备 `smartctl` 与设备权限时采集磁盘 SMART/NVMe 健康。
...
- 通知投递记录保存邮件、钉钉和企业微信的成功/失败原因与重试次数，失败消息可在设置页使用最新凭据重试。
