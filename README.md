...
1. 准备 Docker Engine、Compose v2，以及用户自行维护的外部 MySQL 8.0+。项目不携带 MySQL 容器；请先创建一个空数据库和具备建表、索引权限的数据库账号。

```powershell
docker compose up --build -d
```

2. 打开 `http://<服务器IP>:18080/setup`。首次运行向导第一步填写 MySQL 地址、端口、数据库名、用户名和密码，检测通过后初始化监控表结构；随后设置站点入口和首个管理员。
