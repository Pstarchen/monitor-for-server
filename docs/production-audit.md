...
- `.env` 中外部 `DB_URL`、`DB_USERNAME`、`DB_PASSWORD`、管理员密码和 `SETTINGS_ENCRYPTION_KEY` 不提交 Git；安装器不使用额外的 MySQL 管理账号，数据库账号由用户在向导中明确填写；`SETTINGS_ENCRYPTION_KEY` 丢失会使数据库中的通知凭据不可恢复。
