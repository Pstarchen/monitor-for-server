...
2. 目标库不存在时执行 `CREATE DATABASE`；已有空数据库执行服务端 V1 表结构并写入 Flyway 初始记录；已有数据库会逐项校验监控表和关键字段，避免把不完整旧库当作可用库。
...
向导会在 1130 响应中显示不含密码的授权 SQL 模板，可复制到宝塔数据库终端执行。执行前必须把 `REPLACE_WITH_DATABASE_PASSWORD` 替换为该账号密码；这只新增 `user@'%'`（或之后再收紧到 Docker 网段）的账号，不会修改原有 `user@'localhost'` 账号。
