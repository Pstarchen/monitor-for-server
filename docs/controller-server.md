...
两个入口都固定到 `v1.20.20`，不要替换为可变的 `main`。默认安装目录是 `/opt/guanlan-monitor`，可通过 `--install-dir <绝对路径>` 修改。`CN=true` 直接拉取 `ccr.ccs.tencentyun.com/xc_monitor` 下的六个公开预构建镜像，不在目标机编译；它不代表离线，如果目标机也无法访问 Gitee、腾讯云 TCR 或系统包源，就应使用内部源或已校验的离线 bundle。
...
完全断网的已有部署必须使用离线包内的 `upgrade-offline.sh/.ps1`，不能再次运行新装入口。例如将已有 amd64 部署升级到 `v1.20.20`：

```bash
sha256sum -c xingchen-monitor-offline-v1.20.20-amd64.tar.gz.sha256
tar -xzf xingchen-monitor-offline-v1.20.20-amd64.tar.gz
cd xingchen-monitor-offline-v1.20.20-amd64
