...
- 外部 MySQL 8.0+ 实例，并提前创建一个空数据库及具备建表、索引权限的应用账号；总终端不会安装 MySQL 服务本身，也不会替用户创建数据库或修改 MySQL 用户。
...
浏览器向导运行在总终端的 `setup` 内部服务中，不要求宿主机安装 MySQL 客户端。第一步一次填写 MySQL 访问地址、端口、目标数据库名、用户名和密码并检测连接；检测通过后会初始化监控表结构，然后：

1. 测试数据库账号能否打开目标数据库。
2. 空数据库自动执行服务端 V1 表结构，并写入 Flyway 初始记录；已有完整表结构则直接复用。
3. 将数据库连接写入总终端私有 `.env`，不输出到日志。

如果数据库已有不完整表结构，安装器会停止并提示使用空数据库或先完成迁移，不会删除或覆盖业务数据。生产环境建议按实际网段限制 MySQL 来源并通过防火墙保护 3306。若 MySQL 与总终端同机，容器连接地址使用 `host.docker.internal`，不要填容器内的 `127.0.0.1`。
...
1. 填写 MySQL 地址、端口、目标数据库名、用户名和密码，检测连接并初始化表结构。
2. 设置公网入口、来源、站点名、Web 端口、绑定地址和首个管理员密码。
3. 等待页面显示服务重建完成，再进入登录页。

同机 MySQL 的访问地址通常填写 `host.docker.internal`，不要填写容器内的 `127.0.0.1`。如果暂时没有域名，可以先用 `http://<服务器IP>:18080`；HTTPS 和宝塔反代配置完成后，再在系统设置或 `.env` 中切换为正式域名。
