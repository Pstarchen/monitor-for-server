...
`v1.20.20` 已修复该兼容问题。运行 `v1.20.19` Setup 的总控应先通过受支持的总控更新流程升级到 `v1.20.22`，确认 Setup 运行新镜像后，刷新设备页面并重新复制安装命令。`v1.20.21` 还补齐了[旧 Setup 中 BusyBox 的更新兼容性](#在线更新提示-realpath-不支持-e)。安装器接口优先读取 Setup 镜像内置脚本，只更新宿主机仓库文件不会替换正在下发的旧安装器。沿用原设备记录即可，不需要为此删除设备或数据库；参见 [Agent 版本来源与更新顺序](./monitored-agent.md#版本来源与更新顺序)。
...
## 旧版没有 bootstrap，如何升级到 v1.20.22

`v1.20.18` 及更早版本的 Setup 不含新版在线引导器和更新包，不能仅凭旧控制台有“检查更新”按钮就认定新链路可用。已有部署应按[旧版一次性迁移](./deployment.md#旧版一次性迁移)取得固定 `v1.20.22` 且通过可信 SHA256 校验的入口，再更新既有目录；不要重新安装、执行 `main` 分支脚本或手工改写敏感 `.env`。

没有 `.git` 的旧离线部署也能迁移。显式使用 `--source gitee --version v1.20.22` 会在校验与备份通过后切到 Gitee 版本发现和腾讯云六镜像，并保留数据库与设备配置。迁移成功后优先使用控制台“系统设置 → 系统更新”或 `sudo xingchen update`。
...
`v1.20.22` 使用兼容旧运行环境的路径规范化方式，同时保留路径存在性、符号链接和目录边界检查。配套制品发布后，已配置在线更新的部署可在“系统设置 → 系统更新”选择该版本，或使用：
