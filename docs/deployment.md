...
- 外部 MySQL 8.0+；总终端 Compose 不携带 MySQL 容器。首次运行向导会连接 MySQL，目标数据库不存在时尝试创建，再执行表结构初始化；也可以使用已创建的空数据库和目标库账号。
...
向导第一步要求同时填写 MySQL 地址、端口、目标数据库名、用户名和密码，并分阶段检测连接、检查/创建数据库和初始化表结构；空数据库会初始化服务端 V1 表结构和 Flyway 初始记录，已有完整表结构则复用。随后向导写入生产 `.env`，再自动执行 `docker compose up -d --build server web`。管理员账号在生产数据库迁移后由服务端创建。数据库账号密码只写入总终端私有配置，不会进入日志。发现不完整表结构时会停止，避免覆盖数据。
